...
export default function KitchenPage() {
    ...
    // AI Prep Time Removed - Defaulting to standard time
    useEffect(() => {
        const activeTicketsCount = tickets.filter((t: KitchenTicket) => t.status !== 'DELIVERED').length;
        // Simple heuristic: 10 mins base + 5 mins per active ticket batch
    ...
    return (
        <div className="p-8 bg-crema min-h-screen">
            <header className="flex items-center justify-between mb-12">
                <div>
                    <h1 className="text-5xl font-black tracking-tighter text-piedra uppercase">Panel de Cocina</h1>
                    <p className="text-gris font-bold uppercase text-[10px] tracking-widest mt-2 flex items-center gap-2">
                        ...
                        >
                            <div className="flex justify-between items-start mb-6">
                                <div className="flex items-center gap-4">
                                    <div className="w-14 h-14 bg-piedra text-crema rounded-2xl flex items-center justify-center text-2xl font-black">
                                        {ticket.table_id}
                                    </div>
                                    <div>
                            ...
                            <div className="flex-1 space-y-4 mb-8">
                                {ticket.items.map((item, idx) => (
                                    <div key={idx} className="flex justify-between items-center bg-gray-50/50 p-4 rounded-2xl border border-chocolate/5 gap-3">
                                        <span className="font-bold text-piedra flex-1 break-words leading-tight text-sm">{item.name}</span>
                            ...
                            < button
                                ...
                                className={`w-full py-6 rounded-[2rem] font-black uppercase tracking-widest transition-all shadow-lg flex items-center justify-center gap-3 px-4 text-center ${ticket.status === 'READY' ? 'bg-green-600 text-white hover:bg-green-700' :
                                    ticket.status === 'PREPARING' ? 'bg-chocolate text-crema hover:scale-105' :
                                        'bg-piedra text-crema hover:scale-105'
                                    }`}
                            >
