        ...
        <div className="p-6 md:p-10 max-w-[1600px] mx-auto pb-40">
            ...
            {loading ? (
                ...
                <div className="space-y-12">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        <div className="lg:col-span-1 bg-white rounded-[3rem] p-10 shadow-sm border border-gray-100 h-fit">
                            <div className="flex items-center gap-3 mb-8">
                                <div className="w-10 h-10 bg-red-50 rounded-xl flex items-center justify-center text-red-500"><Receipt size={20} /></div>
                                <h3 className="text-xl font-black text-gray-900 tracking-tight uppercase">Top Gastos</h3>
                            </div>
                            <div className="space-y-4">
                                {Object.entries(stats.expensesByCategory).length === 0 ? (
                                    <p className="text-center py-10 text-gray-300 font-bold uppercase text-[10px]">Sin gastos cargados</p>
                                ) : (
                                    Object.entries(stats.expensesByCategory)
                                        .sort((a,b) => b[1] - a[1])
                                        .map(([cat, amount]) => (
                                            <div key={cat} className="flex justify-between items-center p-4 bg-gray-50/50 rounded-2xl border border-gray-100/50">
                        ...
                        {/* Units Sold Section - Better Grid */}
                        <div className="lg:col-span-2">
                             <div className="flex items-center gap-3 mb-8 ml-2">
                                <div className="w-10 h-10 bg-orange-50 rounded-xl flex items-center justify-center text-orange-500"><ShoppingBag size={20} /></div>
                            ...
                            {Object.keys(stats.productsByCategory).length === 0 ? (
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-20">
                                    {Object.entries(stats.productsByCategory).map(([category, items]) => (
                                        <div key={category} className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 group hover:border-black/10 transition-all">
                                            <div className="flex items-center justify-between mb-6">
                                                <h4 className="text-lg font-black text-gray-900 uppercase tracking-tighter">{category}</h4>
                                                <span className="bg-gray-900 text-white px-3 py-1 rounded-full text-[10px] font-black">
                                                ...
                                                {Object.entries(items)
                                                    ...
                                                    .map(([name, qty]) => (
                                                        <div key={name} className="flex justify-between items-center py-2 border-b border-gray-50 last:border-0">
                                                            ...
                                                            <span className="font-black text-gray-900 text-xs">x{qty}</span>
                                                        </div>
                                                    ))}
                                            </div>
                                        </div>
